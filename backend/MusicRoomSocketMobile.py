...
class MusicRoomSocket(Namespace):
    ...
    def on_connect(self):
        ...
        On connect check if the user was already in a group, if he/she was, update is socket_adress.
        ...
        emit("connection_config", response)
    ...
    def on_join_room(self, room_info):
        ...
        if code_exist:
            ...
            room_object['member'][room_info['id']] = {'id' : room_info['id'], 'socket_address': room_info['socket_address']}
    ...
    def on_request_player_state(self,data):
        emit('request_player_state', {'member_socket_address' :self.flask.session['user_socket_id']}, room = data['leader_socket_address'])
    ...
    def on_get_audio_analysis(self,data):
        print(data)
        audio_analysis = requests.get("https://api.spotify.com/v1/audio-analysis/"+data['id'],headers = {'Authorization': 'Bearer ' + self.util.get_valid_spotify_token()})
